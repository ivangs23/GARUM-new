...
import {
  startOfTodayMadridIso,
  isToday,
  msUntilNextMidnightMadrid,
  madridMidnightISO,
  isSameMadridDay,
} from "../src/format/datetime";

// ── Tests migrados de apps/desktop/tests/unit/today.test.ts ──────────────────
...
// ── Tests adicionales para los alias madridMidnightISO / isSameMadridDay ─────

describe("madridMidnightISO", () => {
  it("returns ISO timestamp at Madrid midnight for given Date (CEST, UTC+2)", () => {
    const date = new Date("2026-05-04T14:30:00Z");
    expect(madridMidnightISO(date)).toBe("2026-05-03T22:00:00.000Z");
  });

  it("handles winter timezone (CET, UTC+1)", () => {
    const date = new Date("2026-01-15T12:00:00Z");
    expect(madridMidnightISO(date)).toBe("2026-01-14T23:00:00.000Z");
  });

  it("handles DST transition day (last Sunday of March)", () => {
    // 2026-03-29 is the spring forward day (CET→CEST). At 12:00 UTC Madrid is
    // already on CEST (UTC+2), so midnight was at 2026-03-28T22:00:00.000Z
    const date = new Date("2026-03-29T12:00:00Z");
    expect(madridMidnightISO(date)).toBe("2026-03-28T22:00:00.000Z");
  });
});

describe("isSameMadridDay", () => {
  it("returns true for two timestamps in the same Madrid day", () => {
    const a = new Date("2026-05-04T08:00:00Z"); // 10:00 Madrid CEST
    const b = new Date("2026-05-04T20:00:00Z"); // 22:00 Madrid CEST
    expect(isSameMadridDay(a, b)).toBe(true);
  });

  it("returns false when timestamps cross Madrid midnight", () => {
    const a = new Date("2026-05-04T20:00:00Z"); // 22:00 Madrid CEST (May 4)
    const b = new Date("2026-05-04T23:00:00Z"); // 01:00 Madrid CEST (May 5)
